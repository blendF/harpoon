...
# Harpoon — Autonomous Stateful Black-Box Pentesting

Harpoon is a fire-and-forget web application penetration testing tool built to automate the entire testing process from start to finish. You provide a target, and it handles discovery, analysis, exploitation handoff, and generates a clear, real-world impact report — reducing manual effort while maintaining professional-grade depth.

## What Changed (Architectural Redesign)

Harpoon now uses a **stateful orchestration architecture** instead of a blind linear chain:

- Central `PipelineStateManager` is the single source of truth for discovered assets.
- WAF detection is multi-signal (headers + behavioral probes).
- Tool execution adapts dynamically (rate limits, tamper scripts, tool skipping).
- Metasploit removed from autonomous execution path.
- Deterministic PoC generation replaces noisy blind exploitation.

## 10-Phase Pipeline

1. Passive Recon (`subfinder`, `crt.sh`, `amass`)
2. Active DNS + Infra Filtering (`dnsx` + conditional `nmap`)
3. HTTP Probing + WAF Detection (`httpx` + WAF engine)
4. Visual Recon (`gowitness`)
5. Directory/Vhost Discovery (`ffuf` primary, `gobuster` targeted)
6. Advanced Crawling + Historical Mining (`katana`, `waybackurls`, `gau`, dedupe)
7. JavaScript Analysis (endpoint extraction + Shannon entropy secret detection)
8. Parameter Discovery (`paramspider`, `arjun`, `ffuf` params)
9. Validation (`nuclei` JSONL, `sqlmap`, conditional `nikto`)
10. Manual Exploitation Handoff + deterministic PoC report output

## Communication Funnel (State Manager)

Each phase writes normalized entities into `harpoon_logs/pipeline_state.json`:

- subdomains
- resolved hosts/IPs
- open ports and service fingerprints
- live URLs and technology tags
- discovered paths, vhosts, and parameters
- WAF status per host
- JS-discovered endpoints and potential secrets
- validation findings and PoC statements

Downstream phases read from state, not from ad-hoc text parsing alone.

## Adaptive WAF Logic

When `WAF_PRESENT == True`:

- `nikto` is skipped
- `ffuf` forced to low-noise mode (`-rate 2`, filtered block pages)
- `nuclei` constrained (`-rl 50 -c 10`)
- `sqlmap` uses tamper scripts (`space2comment,randomcase`)

When no WAF is detected, Harpoon uses higher concurrency.
...
- Sqlmap
- Nuclei
- Nikto (optional, auto-skipped on WAF)
...
```powershell
pip install paramspider arjun uro
```
...
Harpoon prompts for the target and executes all phases automatically.

## Output

Primary outputs:

- `Harpoon_Report.md` — final report
- `harpoon_logs/pipeline_state.json` — normalized state snapshot
- `harpoon_logs/poc_findings.json` — deterministic PoC statements

Additional logs include tool-specific files such as:

- `subfinder_subdomains.jsonl`
- `dnsx_resolved.jsonl`
- `httpx_probe.jsonl`
- `ffuf_dir.json`, `ffuf_vhost.json`, `ffuf_params.json`
- `katana_endpoints.jsonl`
- `sqlmap_scan.txt`
- `nuclei_scan.txt`
- `nikto_scan.txt`
...
All tool commands can be overridden:
...
- `HARPOON_NIKTO`
- `HARPOON_OLLAMA`
- `HARPOON_OLLAMA_MODEL`

## Notes on Exploitation

Metasploit has been intentionally removed from autonomous execution.

Harpoon now focuses on:

- reliable discovery
- validated vulnerability detection
- deterministic proof-of-concept reporting

Phase 10 is an explicit handoff to the human pentester for business-logic abuse, authZ flaws, race conditions, and complex chained attacks.
