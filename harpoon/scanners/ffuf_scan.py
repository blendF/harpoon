...
from harpoon.config import FFUF_CMD, FFUF_DIR_LOG, FFUF_PARAMS_LOG, FFUF_VHOST_LOG, SECLISTS_DNS, SECLISTS_DISCOVERY, SECLISTS_PARAMS
...
def _find_ffuf() -> tuple[list[str], bool]:
    direct = find_cmd("ffuf") or find_cmd(FFUF_CMD.split()[0])
    if direct:
        return [direct], False
    if _wsl_has_ffuf():
        return ["wsl", "ffuf"], True
    return [], False
...
def _resolve_wordlist(candidates: list[Path], uses_wsl: bool) -> str | None:
...
def _tech_extensions(technology_tags: list[str] | None) -> list[str]:
    tags = {t.lower() for t in (technology_tags or [])}
    exts = [".txt", ".bak"]
    if any(t in tags for t in ("iis", "asp.net", "aspnet")):
        exts.extend([".aspx", ".config"])
    if any(t in tags for t in ("php", "wordpress", "laravel")):
        exts.append(".php")
    if any(t in tags for t in ("java", "tomcat", "spring")):
        exts.append(".jsp")
...
async def run_ffuf_dir(
    ...
    argv_prefix, uses_wsl = _find_ffuf()
    if not argv_prefix:
        log_path.parent.mkdir(parents=True, exist_ok=True)
        log_path.write_text(json.dumps({"error": "ffuf not found."}), encoding="utf-8")
        return -1, "ffuf not found; see log."

    wl = _resolve_wordlist(
        [
            SECLISTS_DISCOVERY / "raft-medium-directories-lowercase.txt",
            SECLISTS_DISCOVERY / "directory-list-2.3-medium.txt",
            HARPOON_DIR_WL,
        ],
        uses_wsl,
    )
    ...
    argv = argv_prefix + [
        ...
        wl,
        "-e",
        ",".join(_tech_extensions(technology_tags)),
        "-mc",
        "200,204,301,302,403",
        "-t",
        threads,
    ...
    code, _out, err = await run_tool(argv, log_path.with_suffix(".log"), timeout=timeout)
...
async def run_ffuf_vhost(
    ...
    wl = _resolve_wordlist([SECLISTS_DNS / "subdomains-top1million-5000.txt", SUBDOMAINS_WL], uses_wsl)
    if not wl:
        log_path.parent.mkdir(parents=True, exist_ok=True)
...
async def run_ffuf_params(
    ...
    wl = _resolve_wordlist([SECLISTS_PARAMS / "burp-parameter-names.txt", PARAMS_WL], uses_wsl)
    if not wl:
        log_path.parent.mkdir(parents=True, exist_ok=True)
        log_path.write_text(json.dumps({"error": "Params wordlist not found."}), encoding="utf-8")
        return -1, "ffuf params wordlist not found."

    targets = _collect_fuzz_targets(target_url, ffuf_dir_log, seeded_urls=seeded_urls, max_targets=10)
    threads = "10" if is_cdn else "40"
    per_target_timeout = max(20, timeout // max(len(targets), 1))
    all_results: list[dict] = []

    for idx, url in enumerate(targets):
        for mode, fuzz_url, extra in [("GET", f"{url}?FUZZ=harpoon", []), ("POST", url, ["-X", "POST", "-d", "FUZZ=harpoon"])]:
            ...
            argv.extend(_ffuf_rate_args(is_cdn))
            await run_tool(argv, part_json.with_suffix(".log"), timeout=per_target_timeout)
